...
Default period is `day`. Resolve `--period`, `--date`, and natural-language date aliases through the core Report Date Resolution rules before reading logs. Generate the report from the current user's personal log plus `People` and `WorkItems`, filtering by normalized Personal Log `日期` values and warning on repairable numeric date displays. For periods overlapping 2026-05-21 through 2026-06-18, include the business-goal contribution and risk/warning tracking from the core protocol. Produce `.html` by default; use Markdown only if the user explicitly requests it.
